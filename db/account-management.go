...
func (dbi *dbImp) QueryLogin(c *gin.Context, username string) (string, error) {
	...
	defer cancel()
	result, err := dbi.conn.Query(ctx, "SELECT password FROM users WHERE username LIKE $1;", username)
	defer result.Close()
	if err != nil {
...
func (dbi *dbImp) CreateNewUser(c *gin.Context, newUser *model.User) error {
	...
	_, err := dbi.conn.Exec(ctx, "INSERT INTO users (user_id,username,password,email,name,lastname,createdon,lastlogin,isadmin)  VALUES (nextval('users_user_id_seq'),$1,$2,$3,$4,$5,$6,$7,$8);", newUser.Username, newUser.Password, newUser.Email, newUser.Name, newUser.Lastname, newUser.CreatedOn, newUser.LastLogin, newUser.Isadmin)
...
func (dbi *dbImp) UpdateLastLogin(c *gin.Context, lastLoginTime time.Time, logUsername string) error {
	ctx, cancel := context.WithTimeout(c.Request.Context(), model.TIMEOUT)
	defer cancel()
	_, err := dbi.conn.Exec(ctx, "UPDATE users SET lastlogin = $1 WHERE username = $2;", lastLoginTime, logUsername)
	if err != nil {
		return err
	}
	return nil
...
func (dbi *dbImp) UpdateUserInfo(c *gin.Context, firstname, lastname, username string) error {
	ctx, cancel := context.WithTimeout(c.Request.Context(), model.TIMEOUT)
	defer cancel()
	_, err := dbi.conn.Exec(ctx, "UPDATE users SET name = $1,lastname=$2 WHERE username = $3;", firstname, lastname, username)
	if err != nil {
		return err
	}
	return nil
